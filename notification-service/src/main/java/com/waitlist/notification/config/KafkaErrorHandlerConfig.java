...
public class KafkaErrorHandlerConfig {
    ...
    private DefaultErrorHandler defaultErrorHandler(KafkaTemplate<Object, Object> kafkaTemplate) {
        ...
        // Cap total elapsed time so exactly maxRetries retries are attempted
        backOff.setMaxElapsedTime(maxElapsedTime(initialIntervalMs, 2.0, maxIntervalMs, maxRetries));

        var handler = new DefaultErrorHandler(recoverer, backOff);

        // Poison-pill exceptions: skip retries, route straight to DLT
    ...
    /** Sum of the first {@code retries} exponential intervals, plus one ms — so the
     *  BackOff stops exactly after that many retries rather than continuing indefinitely. */
